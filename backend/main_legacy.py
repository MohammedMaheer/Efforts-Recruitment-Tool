...
AI_ANALYSIS_TIMEOUT = float(os.getenv('AI_ANALYSIS_TIMEOUT', '60.0'))  # 60s for thorough extraction (default was 30s)
MAX_CONCURRENT_REQUESTS = _settings.max_concurrent_requests

# Configure logging — use INFO in production so sync/startup progress is visible
logging.basicConfig(
    level=logging.INFO,
...
async def auto_sync_emails():
    ...
    # FIRST sync on startup: ALWAYS fetch the FULL inbox (no date filter).
    # The dedup mechanism (email_processing_log) skips already-processed emails,
    # so this is safe and ensures no email is ever missed.
    # After the first full sync completes, switch to incremental for efficiency.
    _is_first_sync = True
    _last_email_sync_time = None  # Force full inbox fetch on startup
    ...
    async def _run_single_sync():
        """Execute a single sync cycle (Graph API + IMAP fallback). Called with timeout."""
        global _last_email_sync_time
        ...
        try:
            ...
            if all([client_id, client_secret, tenant_id, primary_email]):
                try:
                    ...
                    if token_data and token_data.get('access_token') and not token_data.get('is_expired', True):
                                ...
                                if needs_ai and analysis_text and len(analysis_text) > 20:
                                    try:
                                        ...
                                        if ai_analysis and ai_analysis.get('quality_score') is not None and ai_analysis.get('quality_score') > 0:
                                            ...
                                            # ── Post-merge data-floor: boost score if merged data is richer ──
                                            ...
                                            # ── Reclassify General using merged skills ──
                        ...
                        filter_query = None
                        if _is_first_sync:
                            sync_max_pages = 99999  # Full inbox: UNLIMITED (fetch ALL emails, even old ones)
                            ...
                            sync_max_pages = 300  # Incremental: 300×50 = 15,000 recent (weeks of email)
                        ...
                        try:
                            ...
                            async for page in graph_service.get_messages_paged(
                                ...
                                # increment consecutive counter only for INCREMENTAL syncs.
                                # First sync: NEVER stop early — fetch entire inbox
                                if page_seen == page_size_actual and not _is_first_sync:
                                    consecutive_all_seen += 1
                                    # Incremental syncs: stop after 3 consecutive seen pages
                                    ...
                                    if consecutive_all_seen >= early_stop_threshold:
                                        logger.info(f"⏭️ Stopping: reached {early_stop_threshold} consecutive pages of old emails")
                                        break
                                else:
                                    if not _is_first_sync:
                                        consecutive_all_seen = 0  # Reset counter for incremental
                                    if page_new > 0:
                            ...
                            # After first full sync, switch to incremental for efficiency
                            if _is_first_sync:
            ...
            # IMAP disabled — Microsoft Graph API (OAuth2) is the only sync method
            if not oauth2_success:
                logger.warning("⚠️ Microsoft Graph sync did not complete — verify Microsoft OAuth2 credentials are configured. IMAP has been removed.")
    ...
    while True:
        try:
            # Multi-instance safety: only lease owner runs sync to avoid duplicate processing/cost.
            has_sync_lease = await _acquire_distributed_lease('lease:auto_sync', ttl_seconds=120)
            if not has_sync_lease:
                await asyncio.sleep(15)
                continue

            # First sync: NO timeout — process entire inbox (can take hours for 5000+ emails)
            # Incremental: 10-minute timeout to prevent stuck cycles
            if _is_first_sync:
                await _run_single_sync()  # No timeout — let it finish the full inbox
            else:
                try:
...
async def _acquire_distributed_lease(key: str, ttl_seconds: int = 120) -> bool:
        ...
        with db_service.get_connection() as conn:
            ...
            if row and row[0]:
                try:
                    ...
                    # Malformed lease value -> override safely
                    pass
    ...
    try:
        ...
        return True  # Fail-open so ingestion doesn't halt completely
...
async def _background_process_candidates(interval_minutes: int = 5):
    ...
    - Processes them using Gemini (thinking disabled for cost savings)
    - Runs every 5 minutes until all candidates are fully analyzed
    """
    await asyncio.sleep(60)  # Wait 1 min for startup + model loading to complete
    
    while True:
        try:
            # Multi-instance safety: run BG processing on one leader instance only.
...
async def lifespan(app: FastAPI):
    ...
    # Set FORCE_RESET_RUN_ID to a unique value (e.g. 20260313-run1) to execute once.
    # The value is persisted in sync_metadata; restarts with the same value are skipped.
        ...
        try:
            ...
            if _already_done:
                ...
                with _reset_svc.get_connection() as _conn:
                    ...
                    # Remove incremental cursor so next sync starts from full inbox.
    ...
    # Launch background candidate processing — runs continuously to process all unprocessed candidates
...
async def _run_candidate_analysis(candidate_id: str, refresh: bool = False):
    ...
    try:
        ...
        # TIER 2: Try Gemini deep analysis (production — always available on Cloud Run)
        if not analysis:
            try:
                from services.gemini_service import get_gemini_service
                gemini_svc = get_gemini_service()
        ...
        # If Gemini extracted a personal email from the resume, update the candidate record
        candidate_email_from_ai = analysis.get('candidate_email', '')
        if candidate_email_from_ai and '@' in candidate_email_from_ai:
            # Only update if current email looks like a portal/generic address
        ...
        # Mark fallback analyses so frontend can show appropriate badge
